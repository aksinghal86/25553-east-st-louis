...
colfunc_grays <- colorRampPalette(c('white', '#bdbdbd', '#949494', '#737373', '#4d4d4d', '#262626'))
...
bins_and_colors <- tibble(breaks = cut(total_pcbs$conc, breaks = c(0, 0.50, 0.75, 1, 5, 10, Inf))) |>
  distinct(breaks) |>
  arrange(breaks) |>
  mutate(labels = factor(c('<0.50', '0.50 - 0.75', '0.75 - 1', '1 - 5', '5 - 10', '>10')),
         cutoff_1 = factor(c(grays[2], grays[10], grays[16], grays[22], reds[11], reds[16])),
         cutoff_0.5 = factor(c(grays[2], grays[6], grays[16], reds[12], reds[16], reds[21])),
         cutoff_0.25 = factor(c(grays[2], grays[10], reds[6], reds[8], reds[11], reds[16])),
         cutoff_0.034 = factor(c(grays[2], reds[2], reds[6], reds[8], reds[11], reds[16])))
...
basemap <- function(cutoff = 1) {
  ...
  mapdata <- sf_total_pcbs |>
    mutate(breaks = cut(max_conc, breaks = c(0, 0.50, 0.75, 1, 5, 10, Inf))) |>
...
basemap(cutoff = 0.5)
ggsave('output/map-filled-parcels-zoomed-out-0.5ppm-max-conc.png', height = 6, width = 8, units = 'in') 
...
zoomed_left(cutoff = 0.5)
ggsave('output/map-filled-parcels-zoomed-in-left-0.5ppm-max-conc.png', height = 4, width = 5, units = 'in')
...
zoomed_right(cutoff = 0.5) 
ggsave('output/map-filled-parcels-zoomed-in-middle-0.5ppm-max-conc.png', height = 4, width = 3, units = 'in') 
...
## LKT -- changed from df to df_by_parcel
## Detections by homolog and for total PCBs
detects <- df_by_parcel |> 
  filter(!str_detect(analyte, 'Total')) |> 
  group_by(analyte_prefix, n_cl) |> 
  summarize(n = n(), 
            detects = sum(detected), 
            pct_detects = detects/n*100) |> 
  arrange(n_cl)

ggplot(detects, aes(x = reorder(analyte_prefix, n_cl), y = pct_detects)) + 
  geom_col() + 
  coord_flip() + 
...
ggplot(totals, aes(x = conc)) + 
  stat_ecdf() + 
  facet_wrap(~ reorder(analyte_prefix, n_cl)) +
...
ggsave('output/ecdf-max.png', height = 6, width = 8, units = 'in') 
...
totals |> 
  group_by(parcel, analyte_prefix, n_cl) |>
  summarize(conc = mean(conc)) |> 
  ungroup() |> 
  ggplot(aes(x = reorder(analyte_prefix, n_cl), y = conc)) + 
...
# Using avgs -- ASSUMING we want to look at detected values, not ND
summ_stats <- totals |> 
  filter(detected>0) %>%
  group_by(parcel, analyte, n_cl, analyte_prefix) |>
  summarize(conc = mean(conc)) |> 
  ungroup() |> 
  group_by(analyte_prefix, n_cl) |> 
  summarize(
  ...
  mutate(across(Min:Max, ~round(.,digits = 4)))
...
summ_stats
flextable::save_as_docx(summ_stats, path = 'output/summary-statistics-max.docx')
